...
class InputBarangScreen(MDScreen):
    dialog = None

    def save(self):
        self.kode       = self.ids.kode_barang_id
        self.kodee      = self.kode.text
        self.kodeee     = str(self.kodee)

        self.nama       = self.ids.nama_barang_id
        ...
        try:
            ...
            for item in data:
                urll = item['url']
            url             = 'http://{}/add_barang.php'.format(urll)
            print(url)
            data           = {'kode_barang':self.kodeee, 'nama_barang':self.namaaa,'jumlah':self.jumlahhh}
